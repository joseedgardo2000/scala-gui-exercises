...
class VentanaGUI extends MainFrame {
  ...
  def reportAndClose(): Unit = {
    ...
    println("'Presionar' esta presionado: " + pressMe.selected)
    println(comField.reactions)
    //sys.exit(0)
  }

  title = "GUI - Components"
  val nameField:TextField = new TextField { columns = 32 }
  val comField:TextField = new TextField { columns = 32 }
  ...
  reactions += {
    case EditDone(`nameField`) =>
      println("Nombre actual: " + nameField.text)
